...
# 从 config.local.js 读取配置
def load_config():
    with open('config.local.js', 'r', encoding='utf-8') as f:
        lines = f.readlines()
    # 手动解析，逐行读取
    config = {}
    for line in lines:
        line = line.strip()
        # 匹配形如: APP_ID: 'value',  或 APP_ID: "value",
        match = re.match(r'^\s*(\w+)\s*:\s*[\'"]([^\'"]+)[\'"],?\s*$', line)
        if match:
            key = match.group(1)
            value = match.group(2)
            config[key] = value
    app_key = config.get('APP_ID') or config.get('ASR_MODEL_ID')
    access_key = config.get('DOUBAN_API_KEY')
    if not app_key or not access_key:
        raise ValueError('config.local.js 中缺少 APP_ID 或 DOUBAN_API_KEY\n'
                         f'解析得到: app_key={app_key}, access_key={access_key}\n'
                         f'请检查格式是否正确')
